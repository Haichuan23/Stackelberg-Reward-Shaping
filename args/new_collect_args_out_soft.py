...
from new_argsearch_soft import ARGS  
import os
import time
import sys
import random
...
parser.add_argument("--time_out", type=str, default=None,
                    help="If set, write average per-prompt wall-clock time for the first "
                         "--time_n successful prompts to this file.")
parser.add_argument("--time_n", type=int, default=50,
                    help="Number of successful prompts to time (default: 50).")
...
for config_num, rc in enumerate(run_configs):
    ...
    with out_path.open("a", encoding="utf-8") as fout:
        for idx in tqdm(range(start_idx, len(truncated_ds))):
            ...
            do_time = (
                args.time_out is not None
                and not timing_written
                and len(prompt_times) < args.time_n
            )
            if do_time:
                t0 = time.time()
            ...
            if do_time:
                prompt_times.append(time.time() - t0)
                if len(prompt_times) >= args.time_n and not timing_written:
                    avg_t = sum(prompt_times) / len(prompt_times)
                    total_t = sum(prompt_times)
                    report = [
                        f"Method: soft2 (new_argsearch_soft2)",
                        f"topk (num samples): {rc['topk']}",
                        f"B: {args.B}",
                        f"alpha: {args.alpha}",
                        f"noise: {args.noise}",
                        f"num_prompts timed: {len(prompt_times)}",
                        f"max_new_token: {args.max_new_token}",
                        f"rm_weight (lambda): {rc['rm_weight']}",
                        f"mode: {rc['mode']}",
                        f"",
                        f"Total wall-clock time (first {len(prompt_times)} prompts): {total_t:.3f} s",
                        f"Average time per prompt:                                    {avg_t:.4f} s",
                    ]
                    time_path = Path(args.time_out)
                    time_path.parent.mkdir(parents=True, exist_ok=True)
                    time_path.write_text("\n".join(report) + "\n", encoding="utf-8")
                    print(f"[INFO] Timing report written to {time_path}")
                    timing_written = True
